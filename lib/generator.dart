...
class Web3ContractGenerator extends Builder {
  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    // var abi = List<Abi>.from(
    //     abiDecoded.map<Abi>((Map<String, dynamic> j) => Abi.fromJson(j)));
    ...
    var output = <String>[
      ...
      '  final ContractAbi \$abi = ContractAbi.fromJson(File("$fileNameWithExt").readAsStringSync(), "$fileNameNoExt");',
...
String abiFuncToMethod(ContractFunction func, {bool nullable = false}) {
  var paramWithNames =
      zip<FunctionParameter, int>(func.parameters, CountForever())
          ...
          .toList();
  var paramTypesAndNames = paramWithNames
      .map((pn) => '${pn.value1.type.runtimeType.toString()} ${pn.value2}');
  ...
  return [
    ...
    ...(func.isConstant
        ? <String>[
            'var result = await \$client.call(sender: _addrOrDefault(from), contract: \$contract, function: _f, params: params, atBlock: BlockNum.current());',
                ...
                ? 'return Tuple${func.outputs.length}(' +
                    [
                      for (var r in zip(
                          ...
                          func.outputs))
                        'result[${r.value1}] as ${r.value2.type.runtimeType.toString()}'
                    ].join(', ') +
                    ');'
                : 'return' +
                    (func.outputs.length == 1
                        ? ' result[0] as ${func.outputs.first.type.runtimeType.toString()}'
                        : '') +
                    ';',
          ]
        : [
            'var finalized = tx.finalize(data: params, from: await wallet.privateKey.extractAddress(), to: \$contract.address);',
...
String abiOutputToReturnType(List<FunctionParameter> outputs) {
  ...
  if (outputs.length == 1) return outputs.first.type.runtimeType.toString();
  ...
  return [
    ...
    outputs.map((o) => o.type.runtimeType.toString()).join(', '),
    '>',
  ].join('');
}
...
// String web3TypeToDart(String ethTypeName) {
//   return ethTypeName;
// }

// class Web3Contract {
//   void main() {
//     var w = Web3ContractBuilder();
//   }
// }
